...
+ (int) alpha: (UIColor*)c {
    return [UIColor component:c k:24];
}

+ (int) red: (UIColor*)c {
    return [UIColor component:c k:16];
}

+ (int) green: (UIColor*)c {
    return [UIColor component:c k: 8];
}

+ (int) blue: (UIColor*)c {
    return [UIColor component:c k: 0];
}

+ (int) component: (UIColor*)c k:(int)k {
    int ci = c.colorInt;
    int FF = 0xFF;
    int m = FF << k;
    return ((ci & m) >> k) & FF;
...
- (UIColor*) makeAlpha: (int)a {
    ...
    int na = (a & 0xFF) << 24;
    int nc = na | rgb;
    return [UIColor colorWithInt:nc];
}

- (NSString *)hex {
    long c = [self colorInt];
