 ...
 *
 * 5. View's transform & frame, if (v.transform != identity) { v.frame = f, bool diff = v.frame != f, diff is true }:(
 * 6. Add gesture detector to UIImageView did not set it as userInteractionEnabled, also add a view can block user's interaction :(
 */
